...
subType sub (ArrowType { dom, cod, meta }) = ArrowType { dom: subType sub dom, cod: subType sub cod, meta }
...
unifyType (HoleType { holeId }) t2 =
  if occurs t2 holeId then
    Nothing
  else
    Just $ singleton holeId t2

unifyType t1 (HoleType hole) = unifyType (HoleType hole) t1

unifyType (ArrowType { dom: dom1, cod: cod1, meta: meta1 }) (ArrowType { dom: dom2, cod: cod2, meta: meta2 }) = do
...
unifyTypeRestricted (HoleType { holeId: holeId1 }) (HoleType hole2@{ holeId: holeId2 }) = if holeId1 == holeId2 then Just $ singleton holeId1 hole2 else Nothing

unifyTypeRestricted t1 (HoleType hole) = unifyTypeRestricted (HoleType hole) t1

unifyTypeRestricted (ArrowType { dom: dom1, cod: cod1, meta: meta1 }) (ArrowType { dom: dom2, cod: cod2, meta: meta2 }) = do
...
subTerm sub (Let { termBind, sign, impl, body, meta }) =
  Let
    { termBind
    , sign: subType sub sign
    , impl: subTerm sub impl
    , body: subTerm sub body
    , meta
    }

subTerm sub (Buf { sign, impl, body, meta }) = Buf { sign: subType sub sign, impl: subTerm sub impl, body: subTerm sub body, meta }

subTerm sub (Data { typeBind, sumItems, body, meta }) = Data { typeBind, sumItems: map (subSumItem sub) sumItems, body: subTerm sub body, meta }

subTerm sub (Match { typeId, term, caseItems, meta }) = Match { typeId, term: subTerm sub term, caseItems: map (subCase sub) caseItems, meta }
