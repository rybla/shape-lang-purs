...
unifyRestricted :: Type -> Type -> Maybe HoleSub
unifyRestricted (HoleType id wea md) t2@(HoleType _ _ _) = Just $ singleton id t2
unifyRestricted t1 (HoleType id wea md) = unifyRestricted (HoleType id wea md) t1
unifyRestricted (ProxyHoleType i1) (ProxyHoleType i2) = if i1 == i2 then Just empty else Nothing
unifyRestricted (ArrowType (Parameter a1 mda1) b1 mdb1) (ArrowType (Parameter a2 mda2) b2 mdb2) = do
  a <- unifyRestricted a1 a2
  b <- unifyRestricted (subType a b1) (subType a b2)
  pure $ union a b
unifyRestricted (DataType i1 md1) (DataType i2 md2) = if i1 == i2 then Just empty else Nothing
unifyRestricted _ _ = Nothing
