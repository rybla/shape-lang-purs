...
subType sub (ArrowType param b md2)
    = ArrowType (subParameter sub param) (subType sub b) md2
...
unifyType (ProxyHoleType i1) (ProxyHoleType i2)
    = if i1 == i2 then Just empty else Nothing
...
subTerm sub (LambdaTerm bind block md)
    = LambdaTerm bind (subBlock sub block) md
...
subBlock sub (Block defs t md)
    = Block (map (subDefinition sub) defs) (subTerm sub t) md

subDefinition :: HoleSub -> Definition -> Definition
subDefinition sub (TermDefinition binds ty t md)
    = TermDefinition binds (subType sub ty) (subTerm sub t) md
subDefinition sub (DataDefintion bind ctrs md)
    = DataDefintion bind (map (subConstructor sub) ctrs) md

subConstructor :: HoleSub -> Constructor -> Constructor
subConstructor sub (Constructor bind args md)
    = Constructor bind (map (subParameter sub) args) md
