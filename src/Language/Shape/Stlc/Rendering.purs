...
-- Render types
type ModuleState
  = { module_ :: Module, gamma :: Context, metaGamma :: MetaContext, wrap_module :: Wrap Module, cursor :: Boolean }

type BlockState
  = { block :: Block, gamma :: Context, alpha :: Type, metaGamma :: MetaContext, wrap_block :: Wrap Block, cursor :: Boolean }

type DefinitionsState
  = { definitions :: List Definition, gamma :: Context, metaGamma :: MetaContext, wrap_definitions :: Wrap (List Definition), cursor :: Boolean }
...
moduleComponent =
  ...
  where
  render st@{ module_, gamma, metaGamma, wrap_module } =
    RecWrap.recModule
      { module_:
          \definitions meta gamma metaGamma wrap_mod wrap_definitions ->
            HH.span
              [ classSyntax st "module_" ]
              [ HH.slot _definitions 0 definitionsComponent { definitions, gamma, metaGamma, wrap_definitions, cursor: false } identity
              ]
      }
      module_
      gamma
...
definitionsComponent = undefined
...
--           HH.span
--             [ classSyntax st "module_" ]
--             [ slotSyntax _definitions 0 $ undefined defs gamma metaGamma wrap_defs ]
--   }
-- mkSyntaxComponent \st ->undefined
...
-- renderParameter =  -- RecWrap.recParameter --   { parameter: --       \alpha meta gamma metaGamma wrap_prm wrap_alpha -> --         mkSyntaxComponent \st ->
--   --           HH.span
...
-- renderTypeId typeId gamma metaGamma =  -- mkSyntaxComponent \st -> --   HH.span --     [ classSyntax st "typeId" ] --     $ [ case typeName of --           TypeName Nothing -> HH.text "_" --           TypeName (Just label) -> HH.text label
--   --       ]
...
-- renderTermId termId gamma metaGamma =  -- mkSyntaxComponent \st -> --   HH.span --     [ classSyntax st "termId" ] --     $ [ case termName of --           TermName Nothing -> HH.text "_" --           TermName (Just label) -> HH.text label --       ] --     <> if shadowIndex > 0 then [ HH.sub_ [ HH.text $ show shadowIndex ] ] else []
--   undefined
--   where
